...
Email, Calendar, Contacts, and Subscription management server with 93 tools.
...
| Tool | Description | Parameters |
|------|-------------|------------|
| `list_calendar_events` | List events | `max_results`, `time_min`*, `time_max`*, `query`, `calendar_id` |
| `get_calendar_event` | Get single event by ID | `event_id`, `calendar_id` |
| `create_calendar_event` | Create event | `summary`, `start_time`*, `end_time`*, `description`, `location`, `attendees`, `color_name`, `reminders`, `calendar_id` |
| `create_recurring_event` | Create recurring event | `summary`, `start_time`*, `frequency`, `recurrence_pattern`*, `interval`, `count`, `until`, `by_day`, `reminders`, `calendar_id` |
| `update_calendar_event` | Update event | `event_id`, `summary`, `start_time`*, `end_time`*, `description`, `location`, `reminders`, `calendar_id` |
...
| `add_travel_buffer` | Add travel time before event | `event_id`, `minutes`, `label`, `calendar_id` |
...
All calendar tools support `calendar_id` parameter (defaults to `"primary"`).

```python
# List available calendars to get IDs
list_calendars()

# Create event on a specific calendar (e.g., shared or secondary calendar)
create_calendar_event(
    summary="Drill Weekend",
    start_time="2026-02-07T08:00:00",
    end_time="2026-02-08T17:00:00",
